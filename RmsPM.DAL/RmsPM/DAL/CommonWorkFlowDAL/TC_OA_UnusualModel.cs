namespace RmsPM.DAL.CommonWorkFlowDAL
{
    using System;

    public class TC_OA_UnusualModel
    {
        private DateTime _ApplayDate;
        private int _Auditing;
        private int _Code;
        private string _CoustomName;
        private string _CoustomType;
        private string _Email;
        private string _ProjectCode;
        private string _ProjectName;
        private string _Reason;
        private string _Remark;
        private string _suggest;
        private string _Telephone;
        private string _UserCode;

        public DateTime ApplayDate
        {
            get
            {
                return this._ApplayDate;
            }
            set
            {
                this._ApplayDate = value;
            }
        }

        public int Auditing
        {
            get
            {
                return this._Auditing;
            }
            set
            {
                this._Auditing = value;
            }
        }

        public int Code
        {
            get
            {
                return this._Code;
            }
            set
            {
                this._Code = value;
            }
        }

        public string CoustomName
        {
            get
            {
                return this._CoustomName;
            }
            set
            {
                this._CoustomName = value;
            }
        }

        public string CoustomType
        {
            get
            {
                return this._CoustomType;
            }
            set
            {
                this._CoustomType = value;
            }
        }

        public string Email
        {
            get
            {
                return this._Email;
            }
            set
            {
                this._Email = value;
            }
        }

        public string ProjectCode
        {
            get
            {
                return this._ProjectCode;
            }
            set
            {
                this._ProjectCode = value;
            }
        }

        public string ProjectName
        {
            get
            {
                return this._ProjectName;
            }
            set
            {
                this._ProjectName = value;
            }
        }

        public string Reason
        {
            get
            {
                return this._Reason;
            }
            set
            {
                this._Reason = value;
            }
        }

        public string Remark
        {
            get
            {
                return this._Remark;
            }
            set
            {
                this._Remark = value;
            }
        }

        public string suggest
        {
            get
            {
                return this._suggest;
            }
            set
            {
                this._suggest = value;
            }
        }

        public string Telephone
        {
            get
            {
                return this._Telephone;
            }
            set
            {
                this._Telephone = value;
            }
        }

        public string UserCode
        {
            get
            {
                return this._UserCode;
            }
            set
            {
                this._UserCode = value;
            }
        }
    }
}


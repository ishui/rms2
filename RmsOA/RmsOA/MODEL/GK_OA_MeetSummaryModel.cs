namespace RmsOA.MODEL
{
    using System;

    public class GK_OA_MeetSummaryModel
    {
        private string _AttendPersons;
        private string _CharterMember;
        private int _Code;
        private string _CodeRule;
        private string _Context;
        private string _Dept;
        private DateTime _MeetEndTime;
        private DateTime _MeetStartTime;
        private string _OtherContext;
        private string _OtherPerson;
        private string _Place;
        private string _PreLeave;
        private string _Recoder;
        private string _SendStatus;
        private string _SmallTitle;
        private string _SortCode;
        private string _Status;
        private string _Submiter;
        private DateTime _SubmitTime;
        private string _Title;
        private string _Type;

        public string AttendPersons
        {
            get
            {
                return this._AttendPersons;
            }
            set
            {
                this._AttendPersons = value;
            }
        }

        public string CharterMember
        {
            get
            {
                return this._CharterMember;
            }
            set
            {
                this._CharterMember = value;
            }
        }

        public int Code
        {
            get
            {
                return this._Code;
            }
            set
            {
                this._Code = value;
            }
        }

        public string CodeRule
        {
            get
            {
                return this._CodeRule;
            }
            set
            {
                this._CodeRule = value;
            }
        }

        public string Context
        {
            get
            {
                return this._Context;
            }
            set
            {
                this._Context = value;
            }
        }

        public string Dept
        {
            get
            {
                return this._Dept;
            }
            set
            {
                this._Dept = value;
            }
        }

        public DateTime MeetEndTime
        {
            get
            {
                return this._MeetEndTime;
            }
            set
            {
                this._MeetEndTime = value;
            }
        }

        public DateTime MeetStartTime
        {
            get
            {
                return this._MeetStartTime;
            }
            set
            {
                this._MeetStartTime = value;
            }
        }

        public string OtherContext
        {
            get
            {
                return this._OtherContext;
            }
            set
            {
                this._OtherContext = value;
            }
        }

        public string OtherPerson
        {
            get
            {
                return this._OtherPerson;
            }
            set
            {
                this._OtherPerson = value;
            }
        }

        public string Place
        {
            get
            {
                return this._Place;
            }
            set
            {
                this._Place = value;
            }
        }

        public string PreLeave
        {
            get
            {
                return this._PreLeave;
            }
            set
            {
                this._PreLeave = value;
            }
        }

        public string Recoder
        {
            get
            {
                return this._Recoder;
            }
            set
            {
                this._Recoder = value;
            }
        }

        public string SendStatus
        {
            get
            {
                return this._SendStatus;
            }
            set
            {
                this._SendStatus = value;
            }
        }

        public string SmallTitle
        {
            get
            {
                return this._SmallTitle;
            }
            set
            {
                this._SmallTitle = value;
            }
        }

        public string SortCode
        {
            get
            {
                return this._SortCode;
            }
            set
            {
                this._SortCode = value;
            }
        }

        public string Status
        {
            get
            {
                return this._Status;
            }
            set
            {
                this._Status = value;
            }
        }

        public string Submiter
        {
            get
            {
                return this._Submiter;
            }
            set
            {
                this._Submiter = value;
            }
        }

        public DateTime SubmitTime
        {
            get
            {
                return this._SubmitTime;
            }
            set
            {
                this._SubmitTime = value;
            }
        }

        public string Title
        {
            get
            {
                return this._Title;
            }
            set
            {
                this._Title = value;
            }
        }

        public string Type
        {
            get
            {
                return this._Type;
            }
            set
            {
                this._Type = value;
            }
        }
    }
}

